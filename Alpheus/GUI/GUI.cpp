...
void MouseCursor::Render()
{
    ...
    // Draw border
    g_Render.Triangle({ x, y }, { x + 25, y + 12 }, { x + 12, y + 25 }, Color::Red(200));
}
...
bool MenuMain::MsgProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (!this->vecChildren.empty() && g_Menu.menuOpened)
    {
        for (auto& it : this->vecChildren)
...
void Checkbox::Render()
{
    ...
    if (*this->bCheckboxValue)
        g_Render.RectFilled(this->rcBox, style.colCheckboxActive);
