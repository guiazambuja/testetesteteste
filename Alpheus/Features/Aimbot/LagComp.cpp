...
{
	int updaterate = g_pCvar->FindVar(SteamStrings("cl_updaterate"))->GetInt();
	ConVar* minupdate = g_pCvar->FindVar(SteamStrings("sv_minupdaterate"));
	ConVar* maxupdate = g_pCvar->FindVar(SteamStrings("sv_maxupdaterate"));

	if (minupdate && maxupdate)
		updaterate = maxupdate->GetInt();

	float ratio = g_pCvar->FindVar(SteamStrings("cl_interp_ratio"))->GetFloat();

	if (ratio == 0)
		ratio = 1.0f;

	float lerp = g_pCvar->FindVar(SteamStrings("cl_interp"))->GetFloat();
	ConVar* cmin = g_pCvar->FindVar(SteamStrings("sv_client_min_interp_ratio"));
	ConVar* cmax = g_pCvar->FindVar(SteamStrings("sv_client_max_interp_ratio"));
