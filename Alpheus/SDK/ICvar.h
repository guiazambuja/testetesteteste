...
public:
	...
	void const ConsoleColorPrintf(const Color& clr, const char* pFormat, ...)
	{
		...
		Utils::GetVFunc<OriginalFn>(this, 25)(this, clr, buf, list);
	}
	typedef void(__cdecl* ColorMsgFn)(Color& color, const char* msg);
	void ColorMsg(Color& color, const char* msg)
	{
		if (msg == nullptr)
			return;
		static ColorMsgFn fn = (ColorMsgFn)GetProcAddress(GetModuleHandleA("tier0.dll"), "?ConColorMsg@@YAXABVColor@@PBDZZ");
		fn(color, msg);
	}
};
